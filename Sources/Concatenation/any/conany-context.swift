...
public struct ConcatenationContext: Sendable, Codable, Equatable {
    ...
    private func indentFollowingLines(
        _ multiLine: String,
        by spaces: Int
    ) -> String {
    ...
    public func header(
        ...
        if let t = title {
            pairs.append(("title", jsonEncodedString(t)))
        }

        if let d = details {
            pairs.append(("details", jsonEncodedString(d)))
        }

        if let deps = dependencies, !deps.isEmpty {
            pairs.append(("dependencies", jsonEncodedArray(deps)))
        }

        if let cf = concatenatedFile {
